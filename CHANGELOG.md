...
### Added
- **APIM `Consumption` tier is supported, and deliberately not the default.** It deploys and validates end to end, costs ~$0 idle against ~$197/month for `Basicv2`, and installs in ~14 min instead of ~25-35 — but the first request after 35 minutes idle took **54 seconds**, measured on the gateway alone (an unauthenticated `401` that never reaches a backend). Recommended for disposable test environments; not for any session with a live audience. The number, the two ways to measure it wrong, and why a keep-alive ping was rejected are recorded in [`docs/06-apim-consumption.md`](labs/ai-foundry-hosted-agents-custom-framework-automation/docs/06-apim-consumption.md) so the experiment does not get repeated from scratch.
- **A patch mechanism for the vendored lab.** `Consumption` needs `sku.capacity: 0`, which upstream's `apim.bicep` hardcodes as `1` and does not expose as a parameter. The change lives in `patches/` and `sync-vendor.ps1` applies it to every sync — before the build check, and failing the sync loudly if a patch stops applying — so `vendor/` stays "upstream plus a known, reviewable delta" instead of accumulating hand edits that the next sync would silently discard.
- **`teardown.ps1` purges soft-deleted resources.** Deleting the resource group only soft-deletes APIM and the Foundry accounts, which keep their names and their regional quota for up to 48h and break the next deployment into a fresh group.

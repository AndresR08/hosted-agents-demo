...
export const translations: Record<Locale, Record<string, string>> = {
  en: {
    ...
    "flow.node.client": "Console",
    "flow.node.agent": "Agent",
    "flow.fact.client": "api-key",
    "flow.fact.apim1": "hosted-agents-responses · managed identity → ai.azure.com",
    "flow.fact.agent": "your container",
    "flow.fact.apim2": "inference · managed identity → cognitiveservices.azure.com",
    "flow.fact.model": "model deployment",
    "flow.seg.gateway": "API Management",
    "flow.seg.agent": "agent",
    "flow.seg.model": "model",
    "flow.derivedMark": "(derived)",
    "flow.legend.gateway": "API Management's own processing",
    "flow.legend.backend": "backend: agent and model",
    "flow.note.idle": "The path is the same for either framework. Ask the agent something to watch a real request travel it.",
    "flow.note.pending": "Animated on the measured total. Per-hop figures appear once Log Analytics ingests the gateway logs — about two and a half minutes — and are never estimated in the meantime.",
    "flow.note.measured": "Per-hop times from ApiManagementGatewayLogs. Segment speeds are proportional to them; the playback is compressed, so the ratios are real and the wall-clock duration is not. Agent processing is arithmetic on two measurements, not a measurement.",

    "seq.title": "How the identity flow works, step by step",
    "seq.subtitle": "One request, two managed-identity tokens, two audiences. Conceptual: this is the mechanism, not a reading of any invocation - the Live tab measures the same path in four segments.",
    "seq.counter": "step {n} of {total}",
    "seq.kind.req": "Request",
    "seq.kind.token": "Identity",
    "seq.kind.self": "Internal",
    "seq.kind.resp": "Response",
    "seq.play": "Play",
    "seq.pause": "Pause",
    ...
    "obs.gov.selectHint": "Select a control to see the observation that proves it.",
  ...
  es: {
    ...
    "flow.node.client": "Consola",
    "flow.node.agent": "Agente",
    "flow.fact.client": "api-key",
    "flow.fact.apim1": "hosted-agents-responses · identidad administrada → ai.azure.com",
    "flow.fact.agent": "tu contenedor",
    "flow.fact.apim2": "inference · identidad administrada → cognitiveservices.azure.com",
    "flow.fact.model": "despliegue de modelo",
    "flow.seg.gateway": "API Management",
    "flow.seg.agent": "agente",
    "flow.seg.model": "modelo",
    "flow.derivedMark": "(derivado)",
    "flow.legend.gateway": "procesamiento propio de API Management",
    "flow.legend.backend": "backend: agente y modelo",
    "flow.note.idle": "El camino es el mismo para cualquiera de los dos frameworks. Pregúntale algo al agente para ver una petición real recorrerlo.",
    "flow.note.pending": "Animado sobre el total medido. Los tiempos por salto aparecen cuando Log Analytics ingesta los registros del gateway — unos dos minutos y medio — y nunca se estiman mientras tanto.",
    "flow.note.measured": "Tiempos por salto de ApiManagementGatewayLogs. La velocidad de cada tramo es proporcional a ellos; la reproducción está comprimida, así que las proporciones son reales y la duración en reloj no. El procesamiento del agente es aritmética sobre dos mediciones, no una medición.",

    "seq.title": "Cómo funciona el flujo de identidad, paso a paso",
    "seq.subtitle": "Una petición, dos tokens de identidad administrada, dos audiencias. Conceptual: es el mecanismo, no la lectura de una invocación - la pestaña En vivo mide ese mismo camino en cuatro tramos.",
    "seq.counter": "paso {n} de {total}",
    "seq.kind.req": "Solicitud",
    "seq.kind.token": "Identidad",
    "seq.kind.self": "Interno",
    "seq.kind.resp": "Respuesta",
    "seq.play": "Reproducir",
    "seq.pause": "Pausar",
    ...
    "obs.gov.selectHint": "Selecciona un control para ver la observación que lo demuestra.",

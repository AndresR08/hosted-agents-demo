...
export const translations: Record<Locale, Record<string, string>> = {
  en: {
    ...
    "flow.note.measured": "Per-hop times from ApiManagementGatewayLogs; hop 2 is tied to hop 1 by timestamp containment — an association, not a single measured transaction. Segment speeds are proportional to them; the playback is compressed, so the ratios are real and the wall-clock duration is not. Agent processing is arithmetic on two measurements, not a measurement.",
    "flow.note.measuredPolicy": "Hop 1 measured by API Management's own policy and returned with the response; hop 2 from ApiManagementGatewayLogs, tied to hop 1 by timestamp containment — an association, not a single measured transaction. Hop 2 did not come back with the response: either this agent's model call did not carry hop 1's trace id, or the gateway cache had no entry for it. Segment speeds are proportional to them; the playback is compressed, so the ratios are real and the wall-clock duration is not. Agent processing is arithmetic on two measurements, not a measurement.",
    "flow.note.tracedPending": "Gateway cost of both hops measured by API Management's own policies and returned with the response. Hop 2 is tied to hop 1 by the W3C trace id both requests carried — one transaction, not a nearby one. The model call streams, and a policy runs when its first byte arrives, not its last: the model's duration, and the agent's with it, come from the gateway log about two and a half minutes later and are never estimated meanwhile.",
    "flow.note.traced": "Gateway cost of both hops measured by API Management's own policies and returned with the response; the model's duration from ApiManagementGatewayLogs, because the call streams. Hop 2 is tied to hop 1 by the W3C trace id both requests carried — one transaction, not a nearby one. Segment speeds are proportional to the measurements; the playback is compressed, so the ratios are real and the wall-clock duration is not. Agent processing is arithmetic on two measurements, not a measurement.",
    "flow.note.tracedPolicy": "Both hops measured by API Management's own policies and returned with the response. Hop 2 is tied to hop 1 by the W3C trace id both requests carried — one transaction, not a nearby one. Segment speeds are proportional to the measurements; the playback is compressed, so the ratios are real and the wall-clock duration is not. Agent processing is arithmetic on two measurements, not a measurement.",
    "flow.note.hop1Policy": "Hop 1 measured by API Management's own policy and returned with the response. Hop 2 — agent to model — reaches the gateway log about two and a half minutes later and is never estimated in the meantime; until then the segments play at equal speed.",
    "flow.note.hop1Log": "Hop 1 from ApiManagementGatewayLogs. Hop 2 — agent to model — has not been matched yet and is never estimated in the meantime; until then the segments play at equal speed.",
    ...
    "journey.gatewayOverheadHop1": "Gateway cost (hop 1)",
    ...
    "obs.correlationMethod.trace-id-log-landed": "Hop 2 is tied to hop 1 by the W3C trace id both requests carried ({traceId}), checked by the gateway at request time — one transaction. The gateway-log row shown for hop 2 is located by timestamp containment, because that log carries no trace id.",
    "obs.correlationMethod.trace-id-log-pending": "Hop 2 is tied to hop 1 by the W3C trace id both requests carried ({traceId}), checked by the gateway at request time — one transaction. Its gateway-log row has not landed yet.",
    "obs.correlationMethod.timestamp-containment": "Hop 2 is associated with hop 1 by timestamp containment — an association, not a single measured transaction. The gateway did not return hop 2 under hop 1's trace id: either this agent's model call did not carry it, or the gateway cache had no entry.",
    "obs.correlationMethod.not-correlated": "Hop 2 is not correlated yet.",
    "obs.stale.notice": "Showing the last good reading, taken at {time} ({age} ago). The latest refresh did not get through; it retries every 20 seconds.",
    "obs.stale.detail": "Technical detail",
    "obs.prompt": "Prompt",
  ...
  es: {
    ...
    "flow.note.measured": "Tiempos por salto de ApiManagementGatewayLogs; el salto 2 se vincula al 1 por contención de marcas de tiempo — una asociación, no una transacción medida. La velocidad de cada tramo es proporcional a ellos; la reproducción está comprimida, así que las proporciones son reales y la duración en reloj no. El procesamiento del agente es aritmética sobre dos mediciones, no una medición.",
    "flow.note.measuredPolicy": "Salto 1 medido por la propia política de API Management y devuelto con la respuesta; salto 2 de ApiManagementGatewayLogs, vinculado al 1 por contención de marcas de tiempo — una asociación, no una transacción medida. El salto 2 no volvió con la respuesta: o la llamada al modelo de este agente no llevó el trace id del salto 1, o la caché del gateway no tenía la entrada. La velocidad de cada tramo es proporcional a ellos; la reproducción está comprimida, así que las proporciones son reales y la duración en reloj no. El procesamiento del agente es aritmética sobre dos mediciones, no una medición.",
    "flow.note.tracedPending": "Costo de gateway de ambos saltos medido por las propias políticas de API Management y devuelto con la respuesta. El salto 2 se vincula al 1 por el trace id W3C que llevaban ambas peticiones — una misma transacción, no una cercana en el tiempo. La llamada al modelo va en streaming, y una política se ejecuta cuando llega su primer byte, no el último: la duración del modelo, y con ella la del agente, llegan del registro del gateway unos dos minutos y medio después y nunca se estiman mientras tanto.",
    "flow.note.traced": "Costo de gateway de ambos saltos medido por las propias políticas de API Management y devuelto con la respuesta; la duración del modelo, de ApiManagementGatewayLogs, porque la llamada va en streaming. El salto 2 se vincula al 1 por el trace id W3C que llevaban ambas peticiones — una misma transacción, no una cercana en el tiempo. La velocidad de cada tramo es proporcional a las mediciones; la reproducción está comprimida, así que las proporciones son reales y la duración en reloj no. El procesamiento del agente es aritmética sobre dos mediciones, no una medición.",
    "flow.note.tracedPolicy": "Ambos saltos medidos por las propias políticas de API Management y devueltos con la respuesta. El salto 2 se vincula al 1 por el trace id W3C que llevaban ambas peticiones — una misma transacción, no una cercana en el tiempo. La velocidad de cada tramo es proporcional a las mediciones; la reproducción está comprimida, así que las proporciones son reales y la duración en reloj no. El procesamiento del agente es aritmética sobre dos mediciones, no una medición.",
    "flow.note.hop1Policy": "Salto 1 medido por la propia política de API Management y devuelto con la respuesta. El salto 2 — del agente al modelo — llega al registro del gateway unos dos minutos y medio después y nunca se estima mientras tanto; hasta entonces los tramos se reproducen a igual velocidad.",
    "flow.note.hop1Log": "Salto 1 de ApiManagementGatewayLogs. El salto 2 — del agente al modelo — aún no se ha asociado y nunca se estima mientras tanto; hasta entonces los tramos se reproducen a igual velocidad.",
    ...
    "journey.gatewayOverheadHop1": "Costo de la puerta de enlace (salto 1)",
    "journey.segment.gateway": "gateway",
    ...
    "obs.correlationMethod.trace-id-log-landed": "El salto 2 se vincula al 1 por el trace id W3C que llevaban ambas peticiones ({traceId}), comprobado por el gateway en el momento de la petición — una misma transacción. La fila del registro del gateway que se muestra para el salto 2 se localiza por contención de marcas de tiempo, porque ese registro no lleva trace id.",
    "obs.correlationMethod.trace-id-log-pending": "El salto 2 se vincula al 1 por el trace id W3C que llevaban ambas peticiones ({traceId}), comprobado por el gateway en el momento de la petición — una misma transacción. Su fila del registro del gateway aún no ha llegado.",
    "obs.correlationMethod.timestamp-containment": "El salto 2 se asocia al 1 por contención de marcas de tiempo — una asociación, no una transacción medida. El gateway no devolvió el salto 2 bajo el trace id del salto 1: o la llamada al modelo de este agente no lo llevaba, o la caché del gateway no tenía la entrada.",
    "obs.correlationMethod.not-correlated": "El salto 2 aún no está correlacionado.",
    "obs.stale.notice": "Mostrando la última lectura correcta, tomada a las {time} (hace {age}). La última actualización no llegó; se reintenta cada 20 segundos.",
    "obs.stale.detail": "Detalle técnico",

...
**Lo que esto dejaba abierto, a propósito, en su momento.** La asignación huérfana del 7 de septiembre para `c15d914a-…` no se eliminó — limpiar una asignación de rol obsoleta de una identidad borrada es una acción separada y deliberada, no un efecto secundario de conceder una nueva, y sigue sin eliminarse. Pero la segunda mitad — "nada en `deploy.ps1` concede esto automáticamente" — se cerró el mismo día; ver justo abajo.

### La brecha se cerró: `Grant-DemoAppServiceRoles` ahora concede el Reader del APIM compartido por sí sola (2026-09-11)

El arreglo manual de arriba se acotó deliberadamente al síntoma en vivo, no a la automatización, y lo dijo explícitamente. Pedido directamente después: incorporar la concesión a `Grant-DemoAppServiceRoles` — mismo rol, mismo alcance estrecho, mismo `Reader`, añadida como una quinta concesión junto al Reader del resource group, AcrPull y Log Analytics Reader — para que una futura recreación del App Service (una identidad system-assigned nueva cada vez, según el propio docstring de `Get-DemoAppServiceName`) no la vuelva a perder.

**No construida sobre `Grant-RoleIfMissing`.** Ese helper es lo que las otras cuatro concesiones ya usan, con éxito, en esta misma sesión y en anteriores — sin razón para mover código ya probado fuera de un comando que funciona. Pero el scope del APIM compartido es exactamente donde vive el hallazgo de `MissingSubscription` de esta misma sección: `az role assignment list`/`create` fallan de forma reproducible contra un recurso `Microsoft.ApiManagement/service` en esta versión del CLI, mientras que la misma forma contra un resource group, un ACR, un workspace de Log Analytics o un proyecto de Foundry funciona. Incorporar la concesión nueva a `Grant-RoleIfMissing` sin modificar habría hecho que una corrida *completa, sin flags* de `deploy.ps1` fallara justo en este paso, en este entorno exacto — peor que la brecha que se quería cerrar. Una función paralela, **`Grant-RoleIfMissingRest`**, tiene el contrato idéntico (idempotente, reintenta solo un principal aún propagándose, falla con lo demás) construida sobre `az rest` GET/PUT en su lugar. `Grant-DemoAppServiceRoles` ganó dos parámetros obligatorios nuevos, `-SharedApimName`/`-SharedApimResourceGroupName`, enlazados desde `$config` en el punto de llamada exactamente como el resto de `deploy.ps1` ya los enlaza.

**Verificado de tres maneras, contra el despliegue `-v2` en vivo, no solo leído:**

1. **`-ValidateOnly` sigue pasando** — pero es la más débil de las tres comprobaciones, y queda registrada como tal: `-ValidateOnly` retorna justo después de validar el Bicep, antes de que el script llegue siquiera a la sección del App Service, así que prueba que el archivo sigue analizándose y que la plantilla de despliegue no se vio afectada, no que la concesión nueva funcione.
2. **El camino idempotente real, llamada directa.** `Grant-DemoAppServiceRoles` invocada directamente (haciendo dot-source de los módulos, sin una corrida completa de `deploy.ps1`) con los valores reales de `-v2` — las cinco concesiones, incluida la nueva, reportaron `(already granted)`. Esto confirma que la lógica de GET-y-coincidencia de `Grant-RoleIfMissingRest` reconoce correctamente la asignación Reader aplicada esa misma mañana, sin duplicado y sin error.
3. **El camino de creación real, ejercitado y limpiado.** `Grant-RoleIfMissingRest` llamada directamente contra un principal genuinamente sin asignar en un scope real (la identidad propia del gateway compartido, AcrPull sobre el ACR de este lab — un `ServicePrincipal` real, así que un ejercicio realista de la rama PUT, pero una concesión que no significa nada y nunca iba a quedarse) — creó una asignación de rol real, confirmada presente vía un GET crudo de `az rest`, y luego eliminada vía `az rest --method delete` y reconfirmada ausente. Ningún residuo quedó en la suscripción.

No ejercitada por separado: la rama de reintento ante `PrincipalNotFound`, que solo se dispara para una identidad creada instantes antes por la misma corrida — las dos únicas identidades disponibles para probar en esta sesión (`a29165e4-…`, `944ee3f2-…`) llevan días existiendo, así que ninguna podía producir ese error. La rama es una copia sin modificar de la lógica ya probada en `Grant-RoleIfMissing`, adaptada al texto de error de `az rest` en vez de lógica nueva, que es lo máximo que se puede decir de ella sin un App Service recién creado contra el cual probarla.
